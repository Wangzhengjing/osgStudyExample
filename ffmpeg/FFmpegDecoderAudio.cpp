...
void FFmpegDecoderAudio::open(AVStream * const stream)
{
    try
    {
        ...
        std::cout<<"FFmpegDecoderAudio::open(..), m_nb_channels="<<m_nb_channels<<", m_context->sample_fmt="<<m_context->sample_fmt<<std::endl;
...
void FFmpegDecoderAudio::run()
{
    ...
    {
        std::cout << "FFmpegDecoderAudio::run : " << error.what() << std::endl;
    }

    catch (...)
    {
        std::cout << "FFmpegDecoderAudio::run : unhandled exception" << std::endl;
    }
}
...
void FFmpegDecoderAudio::setAudioSink(osg::ref_ptr<osg::AudioSink> audio_sink)
{
    ...
    std::cout<<"Assigning "<<audio_sink<<std::endl;
