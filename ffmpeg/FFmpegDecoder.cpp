...
bool FFmpegDecoder::open(const std::string & filename, FFmpegParameters* parameters)
{
    try
    {
        ...
        std::cout <<"start to decode file : "<< filename << std::endl;
        ...
        if (filename.compare(0, 5, "/dev/")==0)
        {
            ...
            std::cout<<"Attempting to stream "<<filename<<std::endl;
            ...
            if (iformat)
            {
                std::cout<<"Found input format: "<<format<<std::endl;
            }
            else
            {
                std::cout<<"Failed to find input format: "<<format<<std::endl;
            }
        ...
        std::cout<< "before video decoder open"<< std::endl;
        std::cout<< "m_video_stream->codec->codec_id : "<< m_video_stream->codec->codec_id<< ":"<< avcodec_get_name(m_video_stream->codec->codec_id)<< std::endl;

        m_video_decoder.open(m_video_stream);
        std::cout<< "after video decoder open"<< std::endl;
        try
        {
        ...
        {
            std::cout << "FFmpegImageStream::open audio failed, audio stream will be disabled: " << error.what() << std::endl;
        }
    }

    catch (const std::runtime_error & error)
    {
        std::cout << "FFmpegImageStream::open : " << error.what() << std::endl;
        return false;
    }
